...
addr_t alloc_mem(uint32_t size, struct pcb_t *proc)
{
	...
	uint32_t num_pages = (size + PAGE_SIZE - 1) / PAGE_SIZE; // Number of pages we will allocate
	...
	if (free_pages >= num_pages && proc->bp + num_pages * PAGE_SIZE <= (1U << ADDRESS_SIZE))
	{
		mem_avail = 1;
	}
	...
	if (mem_avail)
	{
		 ...
		 * 	  valid. */
		int prev_page = -1;
		for (int i = 0, allocated_pages = 0; i < NUM_PAGES && allocated_pages < num_pages; i++)
		{
			if (_mem_stat[i].proc == 0)
			{
				_mem_stat[i].proc = proc->pid;
				_mem_stat[i].index = allocated_pages;
				_mem_stat[i].next = -1;

				if (prev_page != -1)
				{
					_mem_stat[prev_page].next = i; // link to the previous page
				}
				prev_page = i;
				// update segment table
				addr_t first_lv = get_first_lv(ret_mem + allocated_pages * PAGE_SIZE);
				addr_t second_lv = get_second_lv(ret_mem + allocated_pages * PAGE_SIZE);
	...
	{
		printf("We could not allocate new memory region to the process");
	}
