...
int enlist_vm_freerg_list(struct mm_struct *mm, struct vm_rg_struct rg_elmt)
{
  ...
  if ((rg_elmt.rg_start >= rg_elmt.rg_end && rg_elmt.vmaid == 0) || (rg_elmt.rg_start >= rg_elmt.rg_end && rg_elmt.vmaid == 1))
  {
    return -1;
  }
...
int pg_getpage(struct mm_struct *mm, int pgn, int *fpn, struct pcb_t *caller)
{
  ...
  { /* Page is not online, make it actively living */
    ...
    if (MEMPHY_get_freefp(caller->active_mswp, &swpfpn) != 0)
      return -1; // No space in swap area
...
int validate_overlap_vm_area(struct pcb_t *caller, int vmaid, int vmastart, int vmaend)
{
  ...
  for (vmait = caller->mm->mmap; vmait != NULL; vmait = vmait->vm_next)
  {
    ...
    if (vmait->vm_start == vmait->vm_end)
      continue; // empty vm area (do not check)

    if (OVERLAP(vmait->vm_start, vmait->vm_end, cur_vma->vm_start, (unsigned long)vmaend))
    {
      printf("Invalid memory detected!: ");
      printf("[%ld - %ld] and [%ld - %d]\n\n", vmait->vm_start, vmait->vm_end, cur_vma->vm_start, vmaend);
      return -1;
    }
  }
...
int inc_vma_limit(struct pcb_t *caller, int vmaid, int inc_sz, int *inc_limit_ret)
{
  ...
  if (vmaid == 1)
  {
    if (area->rg_end < cur_vma->vm_end) // Increasing size of VMA if allocation is valid
  ...
  *inc_limit_ret = cur_vma->vm_end;
  ...
  {
    if (vm_map_ram(caller, area->rg_end, area->rg_start, cur_vma->sbrk, incnumpage, newrg) < 0)
    {
      free(area);
      return -1; /* Map the memory to MEMRAM */
    }
